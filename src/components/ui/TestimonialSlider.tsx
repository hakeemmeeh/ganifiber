...
export default function TestimonialSlider({ testimonials }: Props) {
  ...
  return (
    <div className="relative max-w-3xl mx-auto overflow-visible px-4">
      ...
      <div className="absolute top-[-30px] left-8 font-serif text-8xl text-electric/[0.04] pointer-events-none select-none select-none z-0">
        “
      </div>

      <div className="bg-white border border-gray-100 rounded-3xl p-8 lg:p-12 shadow-[0_20px_50px_rgba(26,95,240,0.05)] relative z-10 overflow-hidden">
        ...
        {testimonials.map((_, i) => (
          <button
            ...
            className={`transition-all duration-300 cursor-pointer h-2.5 rounded-full ${
              i === activeIndex ? 'w-8 bg-electric' : 'w-2.5 bg-gray-200 hover:bg-gray-300'
            }`}
